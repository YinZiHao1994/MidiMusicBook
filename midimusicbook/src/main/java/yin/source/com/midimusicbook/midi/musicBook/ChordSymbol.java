 ...
 * 一个和弦符号表示在同一时间展示的一组音符.一个和弦包括音符,每个音符的天然符号,和所需的符干.
 * 吐过音符有不同的时间一个独立的和弦可能有两个符干(例如:如果一个音符是四分之一节拍而另一个音符是八分之一节拍)
 */
public class ChordSymbol implements MusicSymbol {
     ...
     */
    private NoteData[] notedata;// 需要绘制的音符数组
    /**
     * The notes to draw
     */
    private AccidSymbol[] accidsymbols;// 将要绘制的天然的符号
    /**
     * The accidental symbols to draw
     */
    private int width;// 和弦的宽度
    /**
     * The width of the chord
     */
    private Stem stem1;// 和弦的符干.可以为空
    /**
     * The stem of the chord. Can be null.
     */
    private Stem stem2;// 和弦的第二个符干.可以为空
    /**
     * The second stem of the chord. Can be null
     */
    private boolean hastwostems;// 此和弦是否有两个符干
    /**
    ...
    public ChordSymbol(ArrayList<MidiNote> midinotes, KeySignature key,
        ...
        starttime = midinotes.get(0).getPulsesOfStartTime();
...
//            }
    ...
    CreateNoteData(ArrayList<MidiNote> midinotes, KeySignature key,
        ...
        for (int i = 0; i < len; i++) {
            ...
            notedata[i].number = midi.getNoteNumber();
            notedata[i].leftside = true;
            notedata[i].whitenote = key.GetWhiteNote(midi.getNoteNumber());
            notedata[i].duration = time.GetNoteDuration(midi.getEndTime() - midi.getPulsesOfStartTime());
            notedata[i].accid = key.GetAccidental(midi.getNoteNumber(), midi.getPulsesOfStartTime() / time.getMeasure());
